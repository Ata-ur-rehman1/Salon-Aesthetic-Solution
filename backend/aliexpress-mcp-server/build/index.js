    ...
    try {
        switch (name) {
            case "search_products": {
                const { keyword, minPrice, maxPrice, shipToCountry, pageNo = 1, pageSize = 20 } = args;
            ...
            case "get_product_details": {
                const { productId, targetLanguage = "EN", targetCurrency = "USD" } = args;
                if (!APP_KEY || !APP_SECRET) {
            ...
            case "get_shipping_info": {
                const { productId, skuId, countryCode, quantity = 1 } = args;
                if (!APP_KEY || !APP_SECRET) {
                    return {
                        content: [
                            {
            ...
            case "create_order": {
                const { skuId, quantity, shippingAddress } = args;
                if (!APP_KEY || !APP_SECRET) {
                    return {
                        content: [
                            {
