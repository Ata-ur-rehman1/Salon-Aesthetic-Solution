...
const HydraMachines = ({ categoryId }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/api/category/694be9d97d6df902a9b3a081/products`);
        setProducts(response.data);
        setLoading(false);
      } catch (error) {
        setError("Our hydra machine collection is currently being updated.");
        setLoading(false);
      }
    };
    fetchProducts();
                ...
                <div className="inline-flex items-center gap-3 px-6 py-3 border rounded-full"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs uppercase tracking-[0.2em] font-medium"
                      style={{ color: colors.textSecondary }}>
                      Showing {products.length} Professional Hydra Machines
                    </span>
                  </div>
                </div>
              </motion.div>
