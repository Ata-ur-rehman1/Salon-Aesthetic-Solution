...
const Steamer = ({ categoryId }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/api/category/69e5463ef5f270d5d903c0f7/products`);
        setProducts(response.data);
        setLoading(false);
      } catch (error) {
        setError("Our steamer collection is currently being updated.");
        setLoading(false);
      }
    };
    fetchProducts();
                ...
                <div className="inline-flex items-center gap-3 px-6 py-3 border rounded-full"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs uppercase tracking-[0.2em] font-medium"
                      style={{ color: colors.textSecondary }}>
                      Showing {products.length} Professional Facial Steamers
                    </span>
                  </div>
                </div>
              </motion.div>
