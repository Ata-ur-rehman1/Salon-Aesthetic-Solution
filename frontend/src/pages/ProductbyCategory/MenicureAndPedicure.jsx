...
import { FaHeart, FaChevronRight, FaShoppingCart, FaSpa, FaStar } from "react-icons/fa";
import { motion } from "framer-motion";
import Loader from "../../components/Loader";
import { useDispatch } from "react-redux";
import { addToCart } from "../../redux/features/cart/cartSlice";
import { toast } from "react-toastify";
import ManicureFallback from "../AdvanceSetting/MenicureandpedicureSofasetup1.webp";

const MenicureAndPedicure = ({ categoryId }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/api/category/69e54614f5f270d5d903c0e7/products`);
        setProducts(response.data);
        setLoading(false);
      } catch (error) {
        setError("Our manicure and pedicure collection is currently being updated.");
        setLoading(false);
      }
    };
    fetchProducts();
  ...
  return (
    <div className="min-h-screen font-sans relative overflow-hidden" style={{ background: colors.bgGradient }}>
      ...
      {/* Products Section */}
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pb-16 sm:pb-24 lg:pb-32">
        {error ? (
          ...
          <div className="text-center py-24 border-t border-b" style={{ borderColor: colors.border }}>
            <div className="inline-flex flex-col items-center gap-4">
              <div className="w-12 h-12 border-2 border-dashed rounded-full flex items-center justify-center"
                ...
                <FaSpa className="text-xl" style={{ color: colors.textTertiary }} />
              </div>
              <div>
                      ...
                      <Link to={`/product/${product._id}`} className="block w-full h-full">
                        ...
                        >
                          <img
                            ...
                            src={product.image || ManicureFallback}
                            alt={product.name}
                          />
                ...
                <div className="inline-flex items-center gap-3 px-6 py-3 border rounded-full"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs uppercase tracking-[0.2em] font-medium"
                      style={{ color: colors.textSecondary }}>
                      Showing {products.length} Professional Manicure & Pedicure Stations
                    </span>
                  </div>
                </div>
              </motion.div>
...
export default MenicureAndPedicure;