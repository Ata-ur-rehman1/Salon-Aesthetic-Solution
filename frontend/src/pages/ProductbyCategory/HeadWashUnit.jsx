...
import { FaHeart, FaChevronRight, FaShoppingCart, FaShower, FaStar } from "react-icons/fa";
...
const HeadWashUnit = ({ categoryId }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/api/category/69e54608f5f270d5d903c0dd/products`);
        setProducts(response.data);
        setLoading(false);
      } catch (error) {
        setError("Our wash station collection is currently being updated.");
        setLoading(false);
      }
    };
    fetchProducts();
          ...
          <div className="text-center py-24 border-t border-b" style={{ borderColor: colors.border }}>
            <div className="inline-flex flex-col items-center gap-4">
              <div className="w-12 h-12 border-2 border-dashed rounded-full flex items-center justify-center"
                ...
                <FaShower className="text-xl" style={{ color: colors.textTertiary }} />
              </div>
              <div>
