...
import Massage3 from "../AdvanceSetting/MassageSPABed3.jpg"
const MassageBed = ({ categoryId }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/api/category/69e545f8f5f270d5d903c0d3/products`);
        setProducts(response.data);
        setLoading(false);
      } catch (error) {
        setError("Our massage bed collection is currently being updated.");
        setLoading(false);
      }
    };
    fetchProducts();
                ...
                <div className="inline-flex items-center gap-3 px-6 py-3 border rounded-full"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs uppercase tracking-[0.2em] font-medium"
                      style={{ color: colors.textSecondary }}>
                      Showing {products.length} Professional Massage Beds
                    </span>
                  </div>
                </div>
              </motion.div>
