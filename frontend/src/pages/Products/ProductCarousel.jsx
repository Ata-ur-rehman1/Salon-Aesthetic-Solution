...
import Logo from "../Auth/salonlogo.png"
...
const ProductCarousel = () => {
  ...
  const NextArrow = (props) => {
    ...
    return (
      <motion.button
        onClick={onClick}
        className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center z-30 bg-white/95 backdrop-blur-sm border border-pink-200 hover:border-pink-400 transition-colors"
      >
        <FaChevronRight className="text-pink-500 text-sm md:text-base hover:text-pink-600" />
      </motion.button>
    );
  };
  ...
  const PrevArrow = (props) => {
    ...
    return (
      <motion.button
        onClick={onClick}
        className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center z-30 bg-white/95 backdrop-blur-sm border border-pink-200 hover:border-pink-400 transition-colors"
      >
        <FaChevronLeft className="text-pink-500 text-sm md:text-base hover:text-pink-600" />
      </motion.button>
    );
  };
    ...
    <div className="w-full overflow-x-hidden">
      <div className="w-full py-8 md:py-12">
        ...
        >
          ...
          <motion.div
            className="h-1 w-12 md:w-20 lg:w-32 mx-auto mb-6 md:mb-8 bg-pink-500"
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
              ...
              <div key={slide.id} className="w-full touch-pan-y">
                ...
                >
                  ...
                  <div className="absolute bottom-8 left-8 md:bottom-16 md:left-16 z-10 space-y-3 md:space-y-4">
                    ...
                    <div className="flex items-center gap-3 text-pink-100 uppercase tracking-[0.2em] text-[10px] md:text-xs font-bold drop-shadow-md">
                      <span>Explore Collection</span> <FaChevronRight size={12} className="text-pink-300" />
                    </div>
                  </div>
                </Link>
              </div>
        ...
        >
          <p className="text-center text-[8px] md:text-[11px] uppercase tracking-[0.3em] text-pink-400 font-bold mb-4 md:mb-7">
            Shop By Category
          </p>
          <div className="grid grid-cols-3 md:grid-cols-7 gap-2 md:gap-3 max-w-4xl mx-auto">
            {CATEGORIES.map((cat, i) => (
              ...
              >
                ...
                >
                  <span className="text-[6.5px] md:text-[11px] uppercase tracking-tighter md:tracking-widest font-semibold leading-none text-gray-700 group-hover:text-pink-600 transition-colors duration-300">
                    {cat.name}
                  </span>
                  <FaChevronRight className="text-[6px] md:text-[8px] text-pink-400 opacity-40 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all duration-300" />
                </Link>
              </motion.div>
            ))}
          </div>
